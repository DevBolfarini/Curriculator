import json
import os
from xhtml2pdf import pisa

def gerar_pdf(dados, empresa):
    """Gera o currículo completo no padrão visual SempreIT"""
    nome = dados.get('nome', 'Denis Bolfarini')
    contato = dados.get('contato', 'denis.bolfarini@gmail.com | 11948103499 | São Paulo, SP')
    resumo = dados.get('resumo', '')
    habilidades = dados.get('habilidades', [])
    experiencias = dados.get('experiencias', [])
    formacao = dados.get('formacao', [])

    html_style = """
    <style>
        @page { size: A4; margin: 1.2cm; }
        body { font-family: Helvetica, Arial, sans-serif; font-size: 9pt; color: #333; line-height: 1.4; }
        .header { border-bottom: 2px solid #1a3a5a; padding-bottom: 5px; margin-bottom: 10px; }
        h1 { color: #1a3a5a; font-size: 18pt; margin: 0; text-transform: uppercase; font-weight: bold; }
        .contato { font-size: 9pt; color: #666; margin-top: 2px; }
        .section-title { 
            color: #1a3a5a; 
            font-weight: bold; 
            font-size: 11pt; 
            border-bottom: 1px solid #ddd; 
            text-transform: uppercase; 
            margin-top: 15px; 
            margin-bottom: 5px; 
        }
        .exp-item { margin-bottom: 10px; }
        .exp-header { font-weight: bold; color: #222; font-size: 10pt; }
        li { margin-bottom: 2px; text-align: justify; }
        ul { margin-top: 3px; }
    </style>
    """

    exp_html = ""
    for exp in experiencias:
        cargo = exp.get('cargo', 'Cargo')
        emp = exp.get('empresa', 'Empresa')
        per = exp.get('periodo', 'Período')
        conquistas = exp.get('conquistas', [])
        bullets = "".join([f"<li>{c}</li>" for c in conquistas])
        exp_html += f"<div class='exp-item'><div class='exp-header'>{cargo} | {emp} ({per})</div><ul>{bullets}</ul></div>"

    hab_html = ", ".join(habilidades)
    form_html = "".join([f"<li>{f}</li>" for f in formacao])

    html_final = f"""
    <html>
    <head>{html_style}</head>
    <body>
        <div class='header'><h1>{nome}</h1><div class='contato'>{contato}</div></div>
        <div class='section-title'>Resumo Profissional</div>
        <p>{resumo}</p>
        <div class='section-title'>Habilidades Técnicas</div>
        <p>{hab_html}</p>
        <div class='section-title'>Experiência Profissional</div>
        {exp_html}
        <div class='section-title'>Formação e Certificações</div>
        <ul>{form_html}</ul>
    </body>
    </html>
    """
    
    os.makedirs("curriculos_gerados", exist_ok=True)
    nome_arquivo = f"CV_Denis_{empresa.replace(' ', '_')}.pdf"
    caminho = os.path.join("curriculos_gerados", nome_arquivo)
    
    with open(caminho, "w+b") as f:
        pisa.CreatePDF(html_final, dest=f)
    return caminho

def obter_prompt(canal, empresa, cargo):
    """Retorna o prompt estruturado para a IA"""
    instr_email = '"email_corpo": "texto para o e-mail"' if "E-mail" in canal else ""
    
    # Usamos aspas simples por fora para não conflitar com as aspas duplas do JSON
    prompt = f'''
    Atue como Especialista em Recrutamento. Analise o PDF e a vaga para {cargo} na {empresa}.
    Retorne RIGOROSAMENTE um JSON com:
    {{
      "nome": "Denis Bolfarini",
      "contato": "denis.bolfarini@gmail.com | 11948103499 | São Paulo, SP",
      "resumo": "Resumo focado em dados",
      "habilidades": ["item1", "item2"],
      "experiencias": [
        {{ "cargo": "título", "empresa": "nome", "periodo": "datas", "conquistas": ["ponto1"] }}
      ],
      "formacao": ["graduação"],
      {instr_email}
    }}
    Mantenha os dados reais do PDF e use KPIs.
    '''
    return prompt